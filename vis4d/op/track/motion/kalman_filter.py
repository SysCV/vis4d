...
class KalmanFilterModule(nn.Module):  # type: ignore # pylint: disable=abstract-method
    """Kalman filter.
    ...
    Args:
        ...
        cov_P0: initial covariance matrix with shape 2Nx2N.
    """
    ...
    def __init__(
        ...
        """Init. Registers arguments as buffers"""
        super().__init__()
...
class KalmanFilterModel(BaseMotionModel):
    """
    Wrapper around the kalman filter module to expose it as generic
     BaseMotionModel
    """

    def __init__(
        self,
        kalman_filter: KalmanFilterModule,
        measurement: torch.tensor,
        num_frames: int,
        motion_dims: int,
        *args: ArgsType,
        **kwargs: ArgsType,
        ...
        super().__init__(num_frames, motion_dims, *args, **kwargs)
        self.kalman_module = kalman_filter
        self.state, self.covariance = self.kalman_module.initiate(measurement)

    def update(self, measurement: torch.tensor, *args, **kwargs) -> None:
        self.state, self.covariance = self.kalman_module.update(
    ...
    def predict(self, *args, **kwargs) -> torch.Tensor:
        self.state, self.covariance = self.kalman_module.predict(
            self.state, self.covariance
        )
        self.time_since_update += 1
        return self.state

    def get_state(self, *args, **kwargs) -> torch.Tensor:
        return self.state

    def get_covariance(self):
        """Returns the covariance matrix (~Uncertainty in state estimate)"""
