...
def content_equal(
    ...
    for line_id in range(  # pylint: disable=consider-using-enumerate
        len(lines1)
    ):
        skip = False
        for prop in ignored_props:
            # Append: to avoid matching a property that is
            # a substring of another property
            prop = prop + ":"
            if prop in lines1[line_id] and prop in lines2[line_id]:
                skip = True
                continue  # ignore these lines

        if not skip and lines1[line_id] != lines2[line_id]:
            print(
                "Line missmatch #",
                line_id,
                ":",
