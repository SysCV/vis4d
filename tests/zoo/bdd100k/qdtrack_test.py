...
class TestQDTrackConfig(unittest.TestCase):
    ...
    config_prefix = "bdd100k.qdtrack"
    gt_config_path = "tests/vis4d-test-data/config_test/bdd100k/qdtrack"
    varying_keys = ["save_prefix", "output_dir", "version", "timestamp"]

    def test_frcnn_r50_fpn_1x_bdd100k(self) -> None:
        """Test the config for QDTrack Faster-RCNN.

        This instantiates the config and compares it to a ground truth.
        """
        compare_configs(
            f"{self.config_prefix}.qdtrack_frcnn_r50_fpn_1x_bdd100k",
