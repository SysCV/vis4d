...
class ReferenceSamplingConfig(BaseModel):
    ...
    def validate_scope(cls, v, values, **kwargs):
        if not v > values['num_ref_imgs'] // 2:
...
class MapTrackingDataset(MapDataset):
    ...
    def __init__(self, sampling_cfg, *args, **kwargs):
        ...
        self.create_video_mapping()
        self.sampling_cfg = sampling_cfg

    def create_video_mapping(self):
        for idx, entry in enumerate(self._dataset):
            self.video_to_idcs[entry['video_id']].append(idx)

    def sample_ref_idcs(self, video_idcs, cur_idx):
        ...
        if self.sampling_cfg.type == 'uniform':
            ...
            valid_inds = frame_ids[left:frame_id] + frame_ids[frame_id + 1:right + 1]
            ref_frame_ids = np.random.choice(valid_inds, self.sampling_cfg.num_ref_imgs, replace=False).tolist()
            ...
            if right <= len(frame_ids):
                ...
                ref_frame_ids = frame_ids[left:frame_id] + frame_ids[frame_id + 1:]
        else:
            raise NotImplementedError(f"Reference view sampling {self.sampling_cfg.type} not implemented.")
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if data is not None:
                ...
                ref_data = [self._map_func(self._dataset[ref_idx], transforms=transforms)[0]
...
class TrackingDatasetMapper(DatasetMapper):
    ...
    def transform_image(self, image, dataset_dict, transforms=None, sem_seg_gt=None):
    ...
    def transform_annotation(self, dataset_dict, transforms):
        ...
        instances.set('track_ids', torch.tensor([anno['instance_id'] for anno in annos], dtype=torch.int))
        return instances
    ...
    def __call__(self, dataset_dict, transforms=None):
        ...
        transforms = self.transform_image(image, dataset_dict, transforms=transforms, sem_seg_gt=sem_seg_gt)
