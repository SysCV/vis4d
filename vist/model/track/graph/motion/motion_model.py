...
class LSTM(nn.Module):
    ...
    def predict(self, velocity, location, hc_0):
        ...
        embed = self.vel2feat(velocity).view(
        ...
        output_pred = (
            self.pred2vel(merge_feat).view(num_batch, self.loc_dim) + location
        )
    ...
    def refine(
        ...
        obs_embed = self.loc2feat(observation).view(
            num_batch, self.feature_dim
        )
        embed = torch.cat([loc_embed, obs_embed], dim=1).view(
            1, num_batch, 2 * self.feature_dim
        )
        ...
        output_pred = (
            self.pred2vel(merge_feat).view(num_batch, self.loc_dim)
            + observation
        )
...
class ConfLSTM(LSTM):
    ...
    def __init__(
        ...
        super(ConfLSTM, self).__init__(
            batch_size, feature_dim, hidden_size, num_layers, loc_dim, dropout
        )
        self.refine_lstm = nn.LSTM(
...
class LocConfLSTM(LSTM):
    ...
    def __init__(
        ...
        super(LocConfLSTM, self).__init__(
            batch_size, feature_dim, hidden_size, num_layers, loc_dim, dropout
        )
        self.refine_lstm = nn.LSTM(
            input_size=3 * feature_dim,
...
class LocTrajLSTM(LSTM):
    ...
    def __init__(
        ...
        super(LocTrajLSTM, self).__init__(
            batch_size, feature_dim, hidden_size, num_layers, loc_dim, dropout
        )
        self.refine_lstm = nn.LSTM(
            input_size=3 * feature_dim,
    ...
    def predict(self, vel_history, refine_loc, pred_loc, hc_0):
        ...
        pred_embed = self.loc2feat(pred_loc).view(
            num_seq, num_batch, self.feature_dim
        )
        refine_embed = self.loc2feat(refine_loc).view(
            num_seq, num_batch, self.feature_dim
        )
        embed = torch.cat(
            [
            ...
            dim=2,
        ).view(num_seq, num_batch, 3 * self.feature_dim)
        ...
        output_pred = (
            torch.sum(attention * vel_history, dim=0) + refine_loc[-1]
        )
